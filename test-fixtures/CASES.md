...
One `.code-workspace` per case, one folder per workspace. `-named` connects to a container whose
`/api/atelier` accepts passwords only; `-anonymous` to one that accepts unauthenticated access only.
