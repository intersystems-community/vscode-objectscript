...
2. **SM resolves** — the Server Manager extension's `getServerSpec` reports the same settings as `webServer` fields, username, password; `auth.resolved()` iff `-named`. For `*-sm` it is looked up by `<serverName>` and is what 1 was built from; for `*-os-*` by folder name, the Servers view's Current node derived from `objectscript.conn`
