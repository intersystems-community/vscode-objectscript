...
In this order, OS being the ObjectScript extension and SM Server Manager as in the case names. Each runs twice: once, then again after idling past the session timeout, so that it is the first request on a lapsed session. `clientSide-os-host` and `clientSide-sm` cases finally run all of them once more with `active` flipped (5 skips the delete then: the extension sends deletes through an `AtelierAPI` it captured when it indexed the folder, so that one keeps the `active` value from activation). A credential prompt anywhere fails the case.
