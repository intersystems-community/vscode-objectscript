...
export class AtelierAPI {
  ...
  /**
   * Get the JWT for use in the Rule Editor iframe.
   */
  public async getRuleEditorJWT(): Promise<any> {
    const { active, host, port, username, password, https } = this.config;
    if (!active || !port || !host) {
      return Promise.reject();
    }

    const path = "/api/interop-editors/login";
    const headers = {
      Accept: "application/json",
    };
    if (typeof username === "string" && typeof password === "string") {
    ...
    const response = await fetch(`${proto}://${host}:${port}${pathPrefix}${path}`, {
      method: "POST",
      agent,
      headers,
    });
    ...
    if (response.status === 401) {
      throw { statusCode: response.status, message: response.statusText, errorText: "Authorization failed." };
    }
    const responseString = new TextDecoder().decode(await response.arrayBuffer());
    let jwt;
    try {
      jwt = JSON.parse(responseString);
    } catch {
      throw {
        statusCode: response.status,
        message: response.statusText,
