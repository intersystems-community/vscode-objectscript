...
| `"intersystems.language-server.diagnostics.suppressSyntaxErrors"` | Controls the languages that syntax error diagnosics will be suppressed for. | `array` | `[]` | Each array element must be one of `"COS"`, `"SQL"`, `"CLS"`, `"HTML"`, `"PYTHON"`, `"XML"`, `"JAVA"`, `"JAVASCRIPT"` or `"CSS"`. |
